    ...
    <div class='total_album'>
        ...
        <div class='select'>
            <div class="area">
                ...
                <span class='b' @click='change' style='background-color: rgb(252, 238, 238); color:#cd0200'>华语</span>
            ...
            <div class="type">
                <span>类型：</span>
                <span class='b' @click='change' style='background-color: rgb(252, 238, 238); color:#cd0200'>全部</span>
                <span class='b' @click='change'>热门</span>
            </div>
            <div class="time">
                <span>时间：</span>
                <el-date-picker v-model="year" type="year" placeholder="选择年" style='margin-right:10px' value-format="yyyy">
                </el-date-picker>
        ...
        <div class='page'>
            <el-pagination @current-change="handleCurrentChange" background layout="prev, pager, next" :total="total" :page-size="45" hide-on-single-page>
            </el-pagination>
        </div>
    </div>
</template>
...
export default {
    ...
    methods: {
        change(e) {
            ...
            this.$api.getAllAlbum(this.now_area, this.now_type, this.year, this.month).then(res => {
    ...
    created() {
        this.$api.getAllAlbum(this.now_area, this.now_type).then(res => {
            this.albums = res.monthData
            this.album = res.monthData.slice(0, 45)
            this.total = res.monthData.length
        }).then(()=>{
    ...
    watch:{
        ...
        value(n){
            this.month=n[2]
            this.$api.getAllAlbum(this.now_area, this.now_type, this.year, n[2]).then(res => {
                console.log(res)
                this.albums = res.monthData
                this.total = res.monthData.length
                this.album = res.monthData.slice(0, 45)
