        ...
        <div class="down">
            <el-tabs v-model="activeName" @tab-click="handleClick" stretch>
                ...
                <el-tab-pane label="MV" name="third">
                    <div class="mv">
                        <mv :pic_url='item.imgurl' :name='item.name' :id='item.id' :art_id='item.artistId' v-for='item in mv' :key='item.id'></mv>
                    </div>
                </el-tab-pane>
                <el-tab-pane label="专辑" name="fourth">
                    <div class="album" v-for='item in album' :key='item.id' @click='toAlbum(item.id)'>
                        <img :src="item.picUrl" alt="">
                        <div class="rig">
                            <p>{{item.name}}</p>
                            <p style='color:grey;font-size:13px;margin-top:10px'>{{item.size}}首</p>
                        </div>
                    </div>
                </el-tab-pane>
    ...
    methods: {
        ...
        toMv(id) {
            this.$api.toMv(id).then(res => {
                console.log(res)
                this.$router.push({ name: 'mv', params: { video_url: res.data.url, a_id: this.id, id } })
            })
        },
        toAlbum(id) {
